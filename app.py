...
app.layout = html.Div(
    ...
    children=[
        dbc.Jumbotron(
            [
                dbc.Fade(html.H1("ALFA Dashboard", className="display-3"), is_in=True,
...
def create_bar_chart(contents):
    if contents is not None:
        ...
        # fig = px.bar(pd.DataFrame(unique_error_frequencies), x="unique_errors", y="frequency", color="dark")
...
@app.callback(Output('output-line-chart', 'children'),
              [Input('upload-data', 'contents')])
def create_bar_chart(contents):
    if contents is not None:
        # create the dataframe of all the data
...
# create numerical data stats facts
